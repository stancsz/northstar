...
Feed in a PRD, get back a full executed test suite and a clear
ship / ship-with-caveats / hold decision.

Most of the pipeline is *classical software QA* — requirement graphs,
equivalence partitioning, boundary value analysis, decision tables, state
transition, pairwise — driven mechanically from the spec. AI-specific layers
(rubric scoring, OWASP LLM Top 10 probes, consistency, agent-trace evaluation)
sit on top, only where they apply, and never float free of a classical
backbone. The reason: an LLM asked to "write tests" produces plausible-looking
happy-path cases and misses the boundaries that actually break software, so
this skill puts classical first and refuses to pass an AI feature that has
rubric tests but no classical coverage.
...
graph, authoring rubrics, scoring AI outputs). Scripts do the deterministic
work (parsing, generating the classical backbone, executing, scoring,
reporting, verifying). The split is the point.
