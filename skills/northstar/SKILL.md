...
| --- | --- | --- |
| **Orchestrator — North Star** | The whole product outcome: understanding the owner, customer/business value, reference products, quality standards, goal priorities, cross-goal dependencies, and shared resource limits | Maintains `docs/northstar/` and the goal index. Chooses and assigns goals within the user's mandate, sets their acceptance and constraints, resolves cross-goal conflicts, and accepts the integrated goal result against the original intent. Owns user communication and the final account of value delivered and remaining gaps. |
| **Supervisor — goal** | One assigned goal from planning through integration, critique, repair, and evidence of completion | Maintains its `docs/goal/<goal>/GOAL.md`. Breaks the goal into bounded tasks, assigns workers and write scopes, manages dependencies and resources within its allocation, accepts task handoffs, and integrates their results. Owns the goal's evaluation coverage and readiness recommendation to the orchestrator. |
| **Worker — task** | One bounded deliverable, including implementation research, execution, relevant checks, cleanup, and an honest handoff | Chooses implementation details within its task. Updates assigned files and evidence in its write scope; returns findings for shared docs to their owner. Reports to its supervisor (or directly to the orchestrator when it fills that role). Cannot broaden the goal, lower acceptance, declare the whole goal complete, or delegate further. |
...
Keep concise task assignments, owners, status, and evidence links in the goal when coordination needs them. Do not create a separate task-document hierarchy by default. Assign one editing owner per shared document. The supervisor consolidates goal evidence in `docs/evals/`; the orchestrator keeps cross-goal conclusions and project direction consistent.
