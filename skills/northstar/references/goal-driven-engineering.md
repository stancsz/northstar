...
2. **Supervisor plans; workers execute:** the supervisor maintains `GOAL.md` and its goal index entry, assigns bounded tasks, write scopes, and report paths, and tracks dependencies. Workers read the goal and relevant North Star constraints, research implementation unknowns, produce coherent slices, check their work, and write `docs/reports/<goal>/<task>.md`. The supervisor reads reports, inspects artifacts and evidence, records task acceptance or repair requests in `GOAL.md`, and integrates the result.
...
5. **Workers repair; supervisor integrates:** fix blocking findings within the agreed scope and submit the updated artifact for review again. Update the same task report, preserving material finding history and refreshed evidence. The supervisor owns integration repairs and recommends readiness only after material findings are resolved and rechecked.
...
Keep task planning and acceptance in the supervisor's goal, and worker handoffs in their reports. Follow the [worker report practice](../SKILL.md#worker-reports-and-handoffs), including for partial or blocked work. The supervisor records the orchestrator's final acceptance decision and updates the goal index; workers cannot close the goal through a report. Use [the Markdown goal template](../templates/GOAL.md) when creating a durable goal.
...
The orchestrator reads and maintains `docs/northstar/` for durable direction and standards. Supervisors read that direction and own their records in `docs/goal/`. Workers produce the actual deliverables and own task reports in `docs/reports/`. Reviewers record what critique and verification found in `docs/evals/`; supervisors ensure coverage and act on findings. Link reports from the goal and keep each fact in one authoritative place. A worker's report is evidence to inspect, not an independent evaluation or acceptance decision.

Only subdirectories belong directly under `docs/`. Put supporting documentation outside those four categories in `docs/misc/`, including installation guides and translations. Keep each index inside its category, and update links when moving existing material.
